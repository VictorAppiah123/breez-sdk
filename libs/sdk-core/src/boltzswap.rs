...
pub enum BoltzApiReverseSwapStatus {
    /// - Boltz: Initial status of a reverse swap
    /// - Breez SDK: Reverse swap created, but HODL invoice not paid (yet)
    #[serde(rename = "swap.created")]
    SwapCreated,

    /// - Boltz: the timelock expires before the HODL invoice is paid
    #[serde(rename = "swap.expired")]
    SwapExpired,

    /// - Boltz: the HODL invoice has been paid (pending settlement), lockup tx is in the mempool
    /// - Breez SDK: Initial status of a reverse swap
    #[serde(rename = "transaction.mempool")]
    LockTxMempool { transaction: LockTxData },

    /// - Boltz: the lockup tx has at least one confirmation
    /// - Breez SDK: When this status is first seen, the claim tx is broadcast
    #[serde(rename = "transaction.confirmed")]
    LockTxConfirmed { transaction: LockTxData },

    /// - Boltz: If Boltz is unable to send the agreed amount of onchain coins after the invoice is
    /// paid, the status will become transaction.failed and the pending lightning HTLC will be cancelled.
    #[serde(rename = "transaction.failed")]
    LockTxFailed,

    /// - Boltz: the HODL invoice was paid, but the timelock expired. In this case, the invoice expires
    /// and the funds are returned to the sender.
    #[serde(rename = "transaction.refunded")]
    #[serde(rename_all = "camelCase")]
    LockTxRefunded { failure_reason: String },

    /// - Boltz: claim tx was seen, HODL invoice was settled
    /// - Breez SDK: Marks the successful completion of a reverse swap
